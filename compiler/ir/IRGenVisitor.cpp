...
antlrcpp::Any IRGenVisitor::visitArray_assign_stmt(ifccParser::Array_assign_stmtContext *ctx)
{
    std::string arrayName = getUniqueName(ctx->ID()->getText());
    std::string index = str(visit(ctx->expr(0)));
    std::string val = str(visit(ctx->expr(1)));

    emit({IRInstr::STORE_ARRAY, arrayName, index, val});
    return 0;
}
...
{
    std::string arrayName = getUniqueName(ctx->ID()->getText());
    std::string index = str(visit(ctx->expr()));
    std::string t = newTemp();

    emit({IRInstr::LOAD_ARRAY, t, arrayName, index});
    return t;
}