...
pld_base_dir_q=shlex.quote(pld_base_dir)
if args.debug:
...
if os.path.isdir(f'{pld_base_dir}/ifcc-test-output'):
    run_command(f'rm -rf {pld_base_dir_q}/ifcc-test-output')

# Ensure that the `ifcc` executable itself is up-to-date
makestatus=run_command(f'cd {pld_base_dir_q}/compiler; make --question ifcc')
if makestatus: # updates are needed
    makestatus=run_command(f'cd {pld_base_dir_q}/compiler; make ifcc',toscreen=True) # this time we run `make` for real
...
if args.S or args.c or args.output:
    ...
    if args.S: # produce assembly
        if args.output[-2:] != ".s":
            ...
            exit(1)
        ifccstatus=run_command(f'{pld_base_dir_q}/compiler/ifcc {inputfilename} > {args.output}')
        if ifccstatus: # let's show error messages on screen
            exit(run_command(f'{pld_base_dir_q}/compiler/ifcc {inputfilename}',toscreen=True))
        else:
            exit(0)
        ...
        ifccstatus=run_command(f'{pld_base_dir_q}/compiler/ifcc {inputfilename} > {asmname}')
        if ifccstatus: # let's show error messages on screen
            exit(run_command(f'{pld_base_dir_q}/compiler/ifcc {inputfilename}',toscreen=True))
...
for jobname in jobs:
    os.chdir(f'{pld_base_dir}/ifcc-test-output')  # os.chdir handles spaces natively
    ...
    ## IFCC compiler
    ifccstatus=run_command(f'{pld_base_dir_q}/compiler/ifcc input.c > ifcc-asm.s', 'ifcc-1-compile.txt')
